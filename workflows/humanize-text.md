---
description: AI生成テキストを「人間らしく」リライトするワークフロー
---

# Humanize AI Text Workflow

このワークフローは、AIが生成した「AIっぽい」文章を、自然な「人間らしい」文章にリライトします。

## 1. 準備

- 作業ディレクトリ: `D:\AG1\humanize-ai-text`
- 出力ディレクトリ: `D:\AG1\humanize-ai-text\output` (存在しない場合は作成)

## 2. 入力ソースの確認

ユーザーに以下のいずれかの方法で対象テキストを入力するよう促してください：

1. **クリップボード**:
   - PowerShellコマンド `Get-Clipboard` を使用してテキストを取得します。
   - 確認メッセージ:「クリップボードの内容を読み込みますか？」
2. **ファイルパス**:
   - ユーザーにファイルパスの入力を求めます。
   - ファイルの内容を `type` または `cat` コマンド等で読み込みます。

## 3. モデルの選択

使用するAIモデルを確認してください：

- **デフォルト**: **Gemini 3.0 Pro**
- **オプション**: **Claude Opus 4.6 (Thinking)**

※ ユーザーから明示的な指定がない場合は、Gemini 3.0 Pro を使用してください。

## 4. リライト実行

取得したテキストに対して、以下のプロンプトを使用してリライトを実行してください。
注意: `{{input_text}}` の部分に読み込んだテキストを挿入してください。
...
リライト結果を以下の形式で保存してください：

1. **ファイル保存**:
   - **拡張子**: `.txt`
   - **ファイル名規則**:
     - **ファイル入力の場合**: `D:\AG1\humanize-ai-text\output\humanized_[元のファイル名(拡張子なし)]_YYYYMMDD.txt`
     - **クリップボードの場合**: `D:\AG1\humanize-ai-text\output\humanized_clipboard_YYYYMMDD.txt`
   - **内容**: リライトされたテキストのみ（Markdown記法なし）

2. **コンソール表示**:
   - ユーザーに完了メッセージと保存パスを表示する。
   - 内容のプレビューを表示する。

3. **クリップボードコピー** (推奨):
   - 結果をクリップボードにコピーすることを提案し、許可されたら実行する (`Set-Clipboard` 等)。
